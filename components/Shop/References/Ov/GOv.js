...
export default function GOv({ serverData, isDovidnuk, setDovActive, setValue }) {
  ...
  const onEdit = () => {
    ...
    if (selectedRowState.length > 0) {
      ...
      //   console.log("Ov/onEdit/agGridId  = ", agGridId)
  ...
  const rowEdit = async (newRow) => {
    // const agGridId = gridRef.current.api.getFirstDisplayedRow() // індекс першого відображеного рядка
    // console.log("/GOv.js/rowEdit/getFirstDisplayedRow= ", agGridId)
  ...
  const { data, mutate, error } = useSWR("/api/shop/references/d_ov/select-all", fetcher)
