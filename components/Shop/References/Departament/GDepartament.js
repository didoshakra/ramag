...
export default function GDepartament({ serverData, isDovidnuk, setDovActive, setValue }) {
  ...
  const rowAdd = async (formData) => {
    // console.log("Departament.js/rowAdd/JSON.stringify(formData)=", JSON.stringify(formData))
    ...
    if (response.ok) {
      ...
      //   console.log(`Departament.js/rowAdd/try/else/ ${err.message} / ${err.stack} `)
    }
  }
  ...
  const rowEdit = async (newRow) => {
    // const agGridId = gridRef.current.api.getFirstDisplayedRow() // індекс першого відображеного рядка
    // console.log("/GDepartament.js/rowEdit/getFirstDisplayedRow= ", agGridId)
  ...
  const onDelete = () => {
    ...
    if (selectedRowState.length > 0) {
      ...
      //   console.log("Departament/onDelete/selRowsID  = ", selRowsID)
  ...
  //= Оновити дані
  const onRefresh = () => {
    // console.log("Departament.js/onRefresh")
    alert("onRefresh")
  ...
  const { data, mutate, error } = useSWR("/api/shop/references/d_departament/select-all", fetcher)
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      ></AgGrid>
      {formActive && <DepartamentForm onCloseForm={onCloseForm} toFormData={toFormData} />}
    </div>
  )
}
