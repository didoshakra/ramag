...
export default function GClient({ serverData, isDovidnuk, setDovActive, setValue }) {
  ...
  const rowAdd = async (formData) => {
    // console.log("Client.js/rowAdd/JSON.stringify(formData)=", JSON.stringify(formData))
    ...
    if (response.ok) {
      ...
      //   console.log(`Client.js/rowAdd/try/else/ ${err.message} / ${err.stack} `)
    }
  }
  ...
  const rowEdit = async (newRow) => {
    // const agGridId = gridRef.current.api.getFirstDisplayedRow() // індекс першого відображеного рядка
    // console.log("/GClient.js/rowEdit/getFirstDisplayedRow= ", agGridId)
  ...
  const onDelete = () => {
    ...
    if (selectedRowState.length > 0) {
      ...
      //   console.log("Client/onDelete/selRowsID  = ", selRowsID)
  ...
  //= Оновити дані
  const onRefresh = () => {
    // console.log("Client.js/onRefresh")
    alert("onRefresh")
  ...
  const onChoose = () => {
    // alert(onChoose)
    // console.log("Client.js/onChoose/SelectedRowState=", selectedRowState["0"])
  ...
  const { data, mutate, error } = useSWR("/api/shop/references/d_client/select-all", fetcher)
