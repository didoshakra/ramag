...
export default function GCategory({ serverData, isDovidnuk, setDovActive, setValue }) {
  ...
  const rowAdd = async (formData) => {
    ...
    if (response.ok) {
      ...
      const data = await mutate("/api/shop/references/d_category/select-all", true) //Обновлення даних
      //   setRowData(data)
    } else {
  ...
  const rowEdit = async (newRow) => {
    // const agGridId = gridRef.current.api.getFirstDisplayedRow() // індекс першого відображеного рядка
    // console.log("/GCategory.js/rowEdit/getFirstDisplayedRow= ", agGridId)
  ...
  const onRefresh = () => {
    ...
    alert("onRefresh")
    mutate("/api/shop/references/d_category/select-all", true) //Обновлення даних SWR
...
if (isDovidnuk) setDovActive("")
  ...
  const { data, mutate, error } = useSWR("/api/shop/references/d_category/select-all", fetcher)
