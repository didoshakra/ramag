...
import UserForm from "../../../components/Shop/References/UserForm"
...
function User({ data, isDovidnuk = false, setDovActive, setValue }) {
  ...
  const toDov = () => {
    ...
    setDovActive("")
    setValue("userId", selectedRowState["0"].id)
    setValue("user", selectedRowState["0"].name)
...
export async function getServerSideProps(context) {
  ...
  //   const response = await fetch("http://localhost:3000/api/shop/references/d_user/select-all")
...
export default function DUser({
  ...
  const { data, error } = useSWR("/api/shop/references/d_user/select-all", fetcher, { initialData: serverData })
