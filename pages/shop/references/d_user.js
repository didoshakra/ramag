...
import GUser from "../../../components/Shop/References/User/GUser"
  ...
  const Dovidnuk = () => {
    return (
      <div style={{ position: "relative", width: "600px", height: "400px", maxWidth: "calc(100vw - 20px)" }}>
        <GUser serverData={serverData} isDovidnuk={isDovidnuk} setDovActive={setDovActive} setValue={setValue} />
      </div>
    )
  }
  const NeDovidnuk = () => {
    return (
      <Layout>
        <div style={{ position: "relative", width: "calc(100vw)", height: "calc(100vh - 150px)" }}>
          <GUser serverData={serverData} isDovidnuk={isDovidnuk} />
        </div>
      </Layout>
    )
  }
