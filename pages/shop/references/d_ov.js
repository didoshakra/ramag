...
import OvForm from "../../../components/Shop/References/OvForm"

function Ov({ data, isDovidnuk = false, setDovActive, setValue }) {
  const titleTable = "Одиниці виміру" //заголовок
  //   console.log("Ov/setValue=", setValue)
  ...
  const columnDefs = useMemo(() => [
    ...
    { field: "kod", headerName: "kod" },
    { field: "name", headerName: "Назва", width: 200, minWidth: 200, flex: 4 },
  ])
  ...
  const onSelectionChanged = (params) => {
    ...
    // console.log("Ov.js/SelectionChanged/К-сть вибраних рядків=", countSelectedRows)
    // console.log("Ov.js/SelectedRowState=", selectedRowState)
  }
  //------------------------------------------------------------------------------------------------------------ */
  const refreshState = () => {
    // console.log("Ov.js/refreshState")
    setRowData(data) //з сервера Pg
  }
  ...
  const onAdd = () => {
    ...
    // rowAdd()// переніс в onCloseForm
  }
  //--- Добавалення(create) запису(запит)
  const rowAdd = async (formData) => {
    // console.log("Ov.js/rowAdd/JSON.stringify(formData)=", JSON.stringify(formData))
    const url = "/api/shop/references/d_ov/insert" //працює
    const options = {
      method: "POST",
    ...
    if (response.ok) {
      ...
      // console.log("Ov.js/rowAdd/try/esponse.ok/rows1: ", rows1);
      // setRows(rows1);
      ...
      //   console.log(`Ov.js/rowAdd/try/else/ ${err.message} / ${err.stack} `)
    }
  }
  ...
  const onEdit = () => {
    if (countSelectedRows > 0) {
      ...
      //   console.log("Ov/onEdit/selectRow  = ", selectRow)
      //****************************************** */
    } else {
  ...
  const rowEdit = async (newRow) => {
    ...
    //Запит до сервера БД
    const url = "/api/shop/references/d_ov/edit" //вибрати все-працює
    const options = {
      method: "PUT",
  ...
  const onDelete = () => {
    if (countSelectedRows > 0) {
      ...
      //   console.log("Ov/onDelete/selRowsID  = ", selRowsID)
  ...
  const rowsDelete = async (rows) => {
    ...
    const url = "/api/shop/references/d_ov/delete" //працює
    const options = {
      method: "DELETE",
  ...
  const toDov = () => {
    // console.log("Ov.js/toDov/SelectedRowState=", selectedRowState["0"])
    setDovActive("")
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      {formActive && <OvForm onCloseForm={onCloseForm} formData={formData} />}
      {/* --- */}
      <style jsx>{`
...
export async function getServerSideProps(context) {
  ...
  //   const response = await fetch("http://localhost:3000/api/shop/references/d_ov/select-all")
...
export default function DOv({
  ...
  // export default function DOv({ serverData }) {
  //   console.log("DOv.js/")
  //= Загрузка даних на фронтенді useSWR ================================================================*/
  const { data, error } = useSWR("/api/shop/references/d_ov/select-all", fetcher, { initialData: serverData })
  ...
  const Dovidnuk = () => {
    return (
      <div style={{ position: "relative", width: "600px", height: "400px", maxWidth: "calc(100vw - 20px" }}>
        <Ov data={data} isDovidnuk={true} setDovActive={setDovActive} setValue={setValue} />
      </div>
    )
  }
  const NeDovidnuk = () => {
