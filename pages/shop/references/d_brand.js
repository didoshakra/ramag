//d_brand.js //Основа- Довідник/НеДовідник\getServerSideProps(context)/useSWR/agGrid\...Form
  ...
  const Dovidnuk = () => {
    return (
      <div style={{ position: "relative", width: "600px", height: "400px", maxWidth: "calc(100vw - 20px)" }}>
        <GBrand data={data} isDovidnuk={true} setDovActive={setDovActive} setValue={setValue} />
      </div>
    )
  }
...
function GBrand({ data, isDovidnuk = false, setDovActive, setValue }) {
  ...
  const onSelectionChanged = (params) => {
    ...
    // console.log("Brand.js/SelectionChanged/К-сть вибраних рядків=", countSelectedRows)
    // console.log("Brand.js/SelectedRowState=", selectedRowState)
  }
  //------------------------------------------------------------------------------------------------------------ */
  const refreshState = () => {
    // console.log("Brand.js/refreshState")
    setRowData(data) //з сервера Pg
  }
  ...
  const rowAdd = async (formData) => {
    ...
    if (response.ok) {
      ...
      // rows1.push(...row); //добавляємо row в кінець масиву rows1
      // console.log("Brand.js/rowAdd/try/esponse.ok/rows1: ", rows1);
      // setRows(rows1);
  ...
  const toDov = () => {
    // console.log("Brand.js/toDov/SelectedRowState=", selectedRowState["0"])
    setDovActive("")
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      {formActive && <BrandForm onCloseForm={onCloseForm} formData={formData} />}
      {/* --- */}
      <style jsx>{`
