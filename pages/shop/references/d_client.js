...
import ClientForm from "../../../components/Shop/References/ClientForm"
import { ComponentContext } from "../../../context/ComponentContext"

function User({ data, isDovidnuk = false, setDovActive, setValue }) {
  const titleTable = "Клієнти" //заголовок
  //   console.log("User/setValue=", setValue)
  ...
  const columnDefs = useMemo(
    () => [
      ...
      { field: "name", headerName: "Ім'я", minWidth: 140, flex: 3 },
  ...
  const onSelectionChanged = (params) => {
    ...
    // console.log("User.js/SelectionChanged/К-сть вибраних рядків=", countSelectedRows)
    // console.log("User.js/SelectedRowState=", selectedRowState)
  }
  //------------------------------------------------------------------------------------------------------------ */
  const refreshState = () => {
    // console.log("User.js/refreshState")
    setRowData(data) //з сервера Pg
  }
  ...
  const rowAdd = async (formData) => {
    ...
    if (response.ok) {
      ...
      // console.log("User.js/rowAdd/try/esponse.ok/rows1: ", rows1);
      // setRows(rows1);
  ...
  const rowEdit = async (newRow) => {
    // console.log("/agrid_clients-pg/rowEdit//newRow= ", newRow)
    //Запит до сервера БД
    const url = "/api/shop/references/d_client/edit" //вибрати все-працює
    const options = {
      method: "PUT",
  ...
  const toDov = () => {
    // console.log("User.js/toDov/SelectedRowState=", selectedRowState["0"])
    setDovActive("")
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      >
        {/* <div style={{ height: "calc(100% - 25px)" }} className={themeDark ? "ag-theme-balham-dark" : "ag-theme-balham"}> */}
        <AgGridReact
...
export async function getServerSideProps(context) {
  ...
  const response = await fetch(`${dbHost}/api/shop/references/d_client/select-all`)
  //   const response = await fetch(`${dbHost}/api/shop/references/d_user/select-all`)
  //   const response = await fetch("http://localhost:3000/api/shop/references/d_client/select-all")
...
export default function DUser({
  ...
  // export default function DUser({ serverData }) {
  //   console.log("DUser.js/")
  //= Загрузка даних на фронтенді useSWR ================================================================*/
  const { data, error } = useSWR("/api/shop/references/d_client/select-all", fetcher, { initialData: serverData })
  ...
  const Dovidnuk = () => {
    return (
      <div style={{ position: "relative", width: "600px", height: "400px", maxWidth: "calc(100vw - 20px" }}>
        <User data={data} isDovidnuk={true} setDovActive={setDovActive} setValue={setValue} />
      </div>
    )
  }
  const NeDovidnuk = () => {
