...
//
import useSWR, { useSWRConfig } from "swr" //https://www.setup.pp.ua/2020/06/useswr-react.html
...
import ProductInsertPasgreSQL from "../../../components/Shop/References/Product/ProductInsertPasgreSQL"
//********************************************************** */
// import "ag-grid-enterprise" //Для EXELL/Дає помилку червоні зірочки, бо не ліцензований
import "ag-grid-community/styles/ag-grid.css"
import "ag-grid-community/styles/ag-theme-alpine.css"
// import "ag-grid-community/dist/styles/ag-theme-balham.css"
import CheckboxRenderer from "../../../components/AgGridModules/CheckboxRenderer"
//
...
export default function DProduct({
  ...
  //   const { data, mutate, error } = useSWR(`${urlAPI}select-all`, fetcher, {
...
function Product({ isDovidnuk = false, setDovActive, setValue, setFocus }) {
  ...
  const onGridReady = (params) => {
    ...
    //  alert("d_product.js/onGridReady")
    if (gridApi) {
      //Якщо включено sizeColumnsToFit, то не працює параметр flex:1/flex:2...
      gridApi.sizeColumnsToFit() //Розмір стовпців відповідно до встановленого розміру(width).Якщо width не встановлено то воно береться з defaultColDef
    }
  ...
  //------------------------------------------------------------------------------------------------------------ */
  const onRefresh = () => {
    // console.log("Product.js/onRefresh")
    mutate(`${urlAPI}select-all`) //повторно перевіряє кешований результат????Не працює
    setRowData(data) //з сервера Pg
  }
  ...
  //
  //= Загрузка даних на фронтенді useSWR ================================================================*/
  const { data, error } = useSWR(`${urlAPI}select-all`, fetcher, {
    //   initialData: serverData,
    refreshInterval: 0,
  })
  //   const { data, error } = useSWR(`${urlAPI}select-all`, fetcher, {  refreshInterval: 30000 })
